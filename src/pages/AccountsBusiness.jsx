...
// Nearly identical to AccountsPersonal but filtered for business accounts
...
import { Plus, Edit2, Trash2, Building2, CreditCard, Globe, Briefcase } from "lucide-react";
...
export default function AccountsBusiness() {
  ...
  const { data: accounts = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      return allAccounts.filter(acc => acc.account_owner_type === 'business');
    },
  });

  const createAccountMutation = useMutation({
    mutationFn: (data) => base44.entities.BankAccount.create({ ...data, account_owner_type: 'business' }),
    onSuccess: () => {
      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
          <div>
            ...
            <p className="text-slate-600 dark:text-slate-300 mt-1">Manage your business bank accounts</p>
          </div>
          <Button 
            onClick={() => {
              resetForm();
        ...
        <Card className="mb-8 border-slate-200 dark:border-slate-700 shadow-md bg-white dark:bg-slate-800">
          <CardHeader>
            <CardTitle className="text-sm font-medium text-slate-600 dark:text-slate-400">Total Business Balance</CardTitle>
          </CardHeader>
          <CardContent>
            <p className="text-3xl font-bold text-slate-900 dark:text-white">
              ${totalBalance.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
            </p>
            <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">{accounts.length} business account{accounts.length !== 1 ? 's' : ''}</p>
          </CardContent>
        </Card>
        ...
        {isLoading ? (
          ...
          <Card className="border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800">
            <CardContent className="flex flex-col items-center justify-center py-16">
              ...
              <h3 className="text-xl font-semibold text-slate-900 dark:text-white mb-2">No business accounts yet</h3>
              <p className="text-slate-600 dark:text-slate-300 text-center mb-6">
                Create your first business bank account to start managing your company finances
              </p>
              <Button 
                onClick={() => {
                  resetForm();
          ...
          <DialogContent className="sm:max-w-[600px] bg-gradient-to-br from-purple-50 to-blue-50 border-purple-200 dark:bg-slate-800 dark:border-slate-700">
            <DialogHeader>
              <DialogTitle className="text-purple-900 dark:text-white">
                {editingAccount ? 'Edit Business Account' : 'Create New Business Account'}
              </DialogTitle>
            </DialogHeader>
                ...
                <div>
                  ...
                  >
                    ...
                    <SelectContent className="bg-purple-50 border-purple-200 dark:bg-slate-800 dark:border-slate-700">
                      ...
                      <SelectItem value="investment" className="text-purple-900 focus:bg-purple-100 dark:text-white dark:hover:bg-slate-700">Investment</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
